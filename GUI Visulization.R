...
ui <- fluidPage(
  ...
  # Application title
  titlePanel("Vachanamrut Visualization"),
...
####   Varnan Cloud Process   ####
  ...
  output$QuesPersonTable <- renderDataTable({
    ...
    write_excel_csv(sheet_data,"Generated Files\\QuestionPerPerson.csv")
    sheet_data <- as.table(as.matrix(sheet_data))
  })
  
  output$QuesPieChart <- renderPlot({
    sheet_data <- read_csv("Resource Files\\Que_Paramahans.csv",col_names = TRUE)
    pie(sheet_data[1:10,]$No_Of_Questions,labels = sheet_data[1:10,]$No_Of_Questions,main = "Questions Per Paramhans", col = rainbow(10),radius = 1)
  ...
  output$paretoChart <- renderPlot({
    sheet_data <- read_csv("Resource Files\\Que_Paramahans.csv",col_names = TRUE)
    sheet_data <- sheet_data %>% filter(sheet_data$No_Of_Questions != 0)
    Paramhans_name <- sheet_data$Paramhans_Name
    No_of_question <- sheet_data$No_Of_Questions
